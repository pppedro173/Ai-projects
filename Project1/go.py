...
class State:
    ...
    def intermediateEvaluation(self,p):
        ...
        if not P1_liberties:
            P1_liberties.append(0)
        for key,values in self.listP2.items():
            P2_liberties.append(self.getLiberties(values))
        if not P2_liberties:
            P2_liberties.append(0)
        if p==1:
            x=min(P1_liberties)-min(P2_liberties)+(sum(P1_liberties)-sum(P2_liberties))
            if min(P1_liberties)==1:
                x-=10
        else:
            x=min(P2_liberties)-min(P1_liberties)+(sum(P2_liberties)-sum(P1_liberties))
            if min(P2_liberties)==1:
                x-=10
        return x
...
class Game:     
    ...
    def actions(self, s):
        ...
        # return sorted(validActions, key=lambda n: s.evaluateAction(n),reverse=True)
...
path='C:/Users/clcas/Desktop/AI/Project1/map.txt'
file=open(path, 'r')
game=Game()
Std=game.load_board(file)
game.initial=Std
print("JOGADA:", prof.alphabeta_cutoff_search(Std, game,3 ))
